...
public class Clock extends View {
    ...
    private Runnable mUpdateClockRunnable = new Runnable() {
        @Override
        public void run() {
            // 刷新视图以更新指针的位置
            invalidate();
    ...
    private void drawHoursValues(Canvas canvas) {
        ...
        for (int i = 1; i <= 12; i++) {
            ...
            float x = (float) (mCenterX + (PANEL_RADIUS - 120) * Math.cos(angle));
            float y = (float) (mCenterY + (PANEL_RADIUS - 120) * Math.sin(angle));
